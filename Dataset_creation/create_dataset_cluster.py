...
class CaptionGenerator:
    ...
    def _generate(self, image: Image.Image, text_prompt: str) -> str:
        ...
        output_ids = self.model.generate(
            **inputs,
            ...
            num_beams=NUM_BEAMS,
            do_sample=False,
            repetition_penalty=1.5,       # should prevent looping single characters/words
            no_repeat_ngram_size=3,       # should prevent looping phrases
        )
    ...
    def generate(self, image: Image.Image, ocr_text: str) -> Tuple[str, str]:
        ...
        try:
            ...
            idx = raw_prompt.lower().find(ocr_text.lower())
            if raw_prompt and idx != -1 and len(raw_prompt) >= 15:
                # BLIP-2 often drops quotes or changes casing. 
                # Inject the correctly cased & quoted text manually.
                before = raw_prompt[:idx]
                after = raw_prompt[idx + len(ocr_text):]
                
                # Strip existing quotes if BLIP-2 actually included them to avoid double quotes
                ...
                training_prompt = f'{before}"{ocr_text}"{after}'
            else:
                # Template fallback
...
class DatasetCreator:
    ...
    def process_sample(self, sample: Dict, subset_name: str) -> Optional[Dict]:
        ...
        # Hard guarantee: final_text must appear in quotes in the prompt
        if not prompt or f'"{final_text}"' not in prompt:
            caption_clean = caption.rstrip(".,;!?").strip()
